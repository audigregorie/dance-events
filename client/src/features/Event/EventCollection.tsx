...
const EventCollection = ({ limit }: { limit?: number }) => {
  ...
  return <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{displayedEvents?.map((event) => <EventCard key={event.id} event={event} />)}</div>;
};
