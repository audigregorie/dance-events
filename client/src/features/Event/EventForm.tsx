...
const EventForm = ({ title, initialData, onSubmit, isLoading }: EventFormProps) => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setFormData((prevFormData: any) => {
      ...
      if (name === EventKeys.TICKET_PRICE) {
        updatedFormData[name] = convertToNumber(value);
      } else {
    ...
    <section className="mt-16 flex justify-center">
      <form onSubmit={handleSubmit} className="flex w-[70rem] flex-col gap-4 rounded-3xl border bg-white p-8 shadow-lg">
        ...
        <div className="grid grid-cols-2 gap-8">
          ...
          {/* Right Column */}
          <div className="flex flex-col gap-4">
            <Input label="Ticket Price" type="number" id={EventKeys.TICKET_PRICE} name={EventKeys.TICKET_PRICE} value={formData.ticket_price?.toString()} onChange={handleChange} className={styles.formInput} required />
